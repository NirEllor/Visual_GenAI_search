...
from exp_config import get_paths, add_exp_arg, ExpPaths
...
def load_real_latents(dim: int, paths: ExpPaths) -> np.ndarray:
    p = paths.real_latent_dir / f"latents_{dim}.npy"
    if not p.exists():
        raise FileNotFoundError(f"Real latents not found: {p}  (run step1 first)")
    return np.load(p).astype(np.float32)


def load_teacher_generated_latents(dim: int, paths: ExpPaths) -> np.ndarray:
    """Load teacher-generated eval latents (step4 --generate --teacher)."""
    p = paths.teacher_latent_dir / f"dim_{dim}" / "z_eval.npy"
    if not p.exists():
        raise FileNotFoundError(
            f"Teacher eval latents not found: {p}  "
            f"(run: python step4_evaluate.py --generate --dim {dim} --teacher)"
        )
    return np.load(p).astype(np.float32)


def load_student_generated_latents(dim: int, size: int, paths: ExpPaths) -> np.ndarray:
    """Load student-generated eval latents (step4 --generate --size N)."""
    p = paths.student_latent_dir / f"dim_{dim}" / f"z_eval_n{size}.npy"
    if not p.exists():
        raise FileNotFoundError(
            f"Student eval latents not found: {p}  "
            f"(run: python step4_evaluate.py --generate --dim {dim} --size {size})"
        )
    return np.load(p).astype(np.float32)
...
def diagonal_gaussian_stats(real: np.ndarray, gen: np.ndarray, eps: float = 1e-8):
    real_mean = real.mean(axis=0);  gen_mean = gen.mean(axis=0)
...
def plot_norm_hist(dim: int, tag: str, real: np.ndarray, gen: np.ndarray, out_dir: Path):
...
def plot_pca_spectrum(dim: int, tag: str, spec, out_dir: Path):
...
def plot_2d(dim: int, tag: str, real: np.ndarray, gen: np.ndarray,
            method: str, n: int, out_dir: Path, seed: int = 0):
    rng = np.random.default_rng(seed)
    ...
    if method == "tsne":
        xy = TSNE(n_components=2, perplexity=40, max_iter=1000,
                  random_state=seed, init="pca", learning_rate="auto").fit_transform(X)
    elif method == "umap":
        import umap
    ...
    plt.xticks([]);  plt.yticks([]);  plt.legend();  plt.tight_layout()
    plt.savefig(out_dir / f"{method}_real_vs_{tag}_dim_{dim}.png", dpi=150)
    plt.close()


def analyze_pair(dim: int, tag: str, real: np.ndarray, gen: np.ndarray,
                 method: str, n_samples: int, out_dir: Path):
    ...
    plot_norm_hist(dim, tag, real_s, gen_s, out_dir)
    plot_pca_spectrum(dim, tag, spec, out_dir)
    if method in ["tsne", "all"]:
        plot_2d(dim, tag, real_s, gen_s, method="tsne", n=n_samples, out_dir=out_dir)
    if method in ["umap", "all"]:
        try:
            plot_2d(dim, tag, real_s, gen_s, method="umap", n=n_samples, out_dir=out_dir)
...
def main():
    ...
    out_dir = paths.exp_dir / "analysis" / "student_latents"
    ...
    for dim in args.dims:
        print(f"\n=== dim={dim} ===")
        real = load_real_latents(dim, paths)
        dim_summary = {}

        if args.include_teacher:
            print("  analyzing teacher")
            try:
                teacher = load_teacher_generated_latents(dim, paths)
                dim_summary["teacher"] = analyze_pair(
                    dim, "teacher", real, teacher, args.method, args.n_samples, out_dir
                )
            except FileNotFoundError as e:
                print(f"  [skip] {e}")

        for size in args.sizes:
            print(f"  analyzing student n={size:,}")
            try:
                student = load_student_generated_latents(dim, size, paths)
            except FileNotFoundError as e:
                print(f"  [skip] {e}")
                continue
            dim_summary[str(size)] = analyze_pair(
                dim, f"student_{size}", real, student, args.method, args.n_samples, out_dir
            )
            print(json.dumps(dim_summary[str(size)], indent=2))
